...
Stripe::Stripe(const float *vertsPositions, const float *directions, const uint32_t *triangleIndices, uint32_t vertsCount, uint32_t triangleCount, float width, Mode mode, bool quiet)
        : width(width), mode(mode) {
    Subdivide subdividedMesh(vertsPositions, mode == Mode::Extrinsic ? nullptr : directions, mode == Mode::Extrinsic ? directions : nullptr, triangleIndices, vertsCount, triangleCount, 0.5f*width, quiet, mode != Mode::Printing);
