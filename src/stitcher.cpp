...
Stitcher::Stitcher(const float *vertsPosition, const float *vertsNormal, const uint32_t *triangles, uint32_t vertsCount, uint32_t facesCount, const std::set<std::array<uint32_t, 2>>& borderEdges, const SDF& sdf)
        : vertsCount(vertsCount), facesCount(facesCount), borderEdges(borderEdges), sdf(&sdf) {
...
void Stitcher::stitch(const float *vertsScalars, float period, bool holeClosing, bool showProgress) {
    Parallel::For(0, vertsCount, [this, vertsScalars](size_t i) {
        scalars[i] = vertsScalars[i];
    });
    doHoleClosing = holeClosing;
    cleanIsolated();
    bool success = false;
    bool first = true;
...
bool Stitcher::connectRegions(float period, bool showProgress) {
    ...
    while (positiveRegion > connectedRegionCount || negativeRegion > connectedRegionCount) {
        ...
        if (showProgress) {
            printProgress(1.0f-(positiveRegion+negativeRegion-2*connectedRegionCount)/total);
        }
    }
    return true;
}
...
bool Stitcher::connectRegion(float period, bool positives) {
    ...
    uint32_t startId = Parallel::ArgMin(0, vertsCount, [this, test](size_t i) {
        float bias = sdf->getCurvature(Vec3(position.data(), i)) > 0.1 ? 0.0 : 1e4;
        return test(scalars[i]) ? distances[i][0][0] + distances[i][1][0] + bias : std::numeric_limits<float>::infinity();
    });
...
void Stitcher::widthenStitch(float period, bool positives, uint32_t id) {
    ...
    for (auto i : pathIds) {
        bool border = doHoleClosing && (sdf->getVal(Vec3(position.data(), i)) > -width && fabsf(sdf->getGrad(Vec3(position.data(), i)).dot(Vec3(normals.data(), i))) < 0.7);
        distances[i][0][0] = border ? width : 0.0f;
        for (auto n : getNeighborhood(i)) {
            uint32_t j = n.id;
            queue.push_back(j);
            explored.emplace(j);
            distances[j][0][0] = distances[i][0][0] + (Vec3(position.data(), i)-Vec3(position.data(), j)).length();
        }
    }
    while (!queue.empty()) {
