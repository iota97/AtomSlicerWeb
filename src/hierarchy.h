...
public:
    MeshHierarchy(const float *vertsPositions, const uint32_t *triangleIndices, uint32_t vertsCount, uint32_t triangleCount) : vertsCount(vertsCount) {
        ...
        for (uint32_t i = 0; i < triangleCount; ++i) {
            ...
            vertex[v0].normal += n/(l.x + l.z) * (vertex[v0].normal.dot(n) < 0 ? -1 : 1);
            vertex[v1].normal += n/(l.y + l.x) * (vertex[v1].normal.dot(n) < 0 ? -1 : 1);
            vertex[v2].normal += n/(l.z + l.y) * (vertex[v2].normal.dot(n) < 0 ? -1 : 1);
        }
