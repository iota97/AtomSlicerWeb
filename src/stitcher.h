...
class Stitcher {
public:
    Stitcher(const float *vertsPosition, const float *vertsNormal, const uint32_t *triangles, uint32_t vertsCount, uint32_t facesCount, const std::set<std::array<uint32_t, 2>>& borderEdges, const SDF& sdf);
    Stitcher() = default;
    
    void stitch(const float *vertsScalars, float period, bool holeClosing, bool showProgress = false);
    void repulse();
    ...
    const std::set<std::array<uint32_t, 2>>& getBorderEdges() { return borderEdges;  }
...
private:
    ...
    const SDF *sdf = nullptr;
    bool doHoleClosing = false;
};